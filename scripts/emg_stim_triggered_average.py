...
pdf_path = data_path / f"Blocks_{blocks_list_str}_emg_stim_triggered_average.pdf"

show_plots = False
with PdfPages(pdf_path) as pdf:
    ###
    plot_emg = emg_df.stack().to_frame(name='signal').reset_index()
    elec_subset = ['-(2,)+(3,)', '-(3,)+(2,)',]  # plot_emg['elecConfig_str'].unique().tolist()
    label_subset = ['LVL', 'LTA', 'LMG', 'RLVL', 'RTA', 'RMG'] # plot_emg['label'].unique().tolist() , 'LSOL' , 'RMH' , 'RSOL'
    ###
    ...
    vert_offset = 0.25 * (plot_emg['signal'].max() - plot_emg['signal'].min())
    ...
    vert_offset = 1e-2 * (plot_emg['signal'].max() - plot_emg['signal'].min())
    horz_offset = 1e-2 * (plot_emg['time_sec'].max() - plot_emg['time_sec'].min())
    n_offsets = 0
