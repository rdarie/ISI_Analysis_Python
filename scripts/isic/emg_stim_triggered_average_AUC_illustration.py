...
from isicpy.lookup_tables import emg_montages, emg_hue_map
...
snsRCParams = {
        ...
        "axes.labelsize": 6 * font_zoom_factor,
        "axes.titlesize": 7 * font_zoom_factor,
        "xtick.labelsize": 5 * font_zoom_factor,
        "ytick.labelsize": 5 * font_zoom_factor,
        "legend.fontsize": 5 * font_zoom_factor,
        "legend.title_fontsize": 5 * font_zoom_factor,
        "xtick.bottom": True,
        "xtick.top": False,
        "ytick.left": True,
        "ytick.right": False,
...
mplRCParams = {
    'figure.titlesize': 7 * font_zoom_factor,
    'pdf.fonttype': 42,
    'ps.fonttype': 42,
    }
sns.set(
...
for block_idx in blocks_list:
    ...
    for timestamp in tqdm(align_timestamps.to_numpy()):
        ...
        aligned_raw_dfs[timestamp] = pd.DataFrame(
            raw_emg.loc[this_mask, :].to_numpy(),
            index=epoch_t, columns=this_emg.columns)
    all_aligned_emg[block_idx] = pd.concat(aligned_dfs, names=['timestamp_usec', 'time_usec'])
    all_aligned_raw_emg[block_idx] = pd.concat(aligned_raw_dfs, names=['timestamp_usec', 'time_usec'])
...
raw_emg_df = pd.concat(all_aligned_raw_emg, names=['block', 'timestamp_usec', 'time_usec'])
raw_emg_df.drop(['L Forearm', 'R Forearm', 'Sync'], axis='columns', inplace=True)
...
raw_emg_df.index = pd.MultiIndex.from_frame(emg_metadata)
#### outlier removal
...
raw_emg_df = raw_emg_df.loc[~outlier_mask, :]
####
if folder_name == 'Day11_PM':
    emg_df.sort_index(level='elecConfig_str', key=reorder_fun, inplace=True)
    raw_emg_df.sort_index(level='elecConfig_str', key=reorder_fun, inplace=True)
...
with PdfPages(pdf_path) as pdf:
    if x_axis_name in ['freq', 'freq_late']:
        recruitment_mask = (emg_df.index.get_level_values('amp') >= amp_cutoff) & (emg_df.index.get_level_values('freq') == freq_cutoff)
        hue_var = 'freq'
    if x_axis_name == 'amp':
    ...
    plot_raw_emg = raw_emg_df.loc[recruitment_mask, :].stack().to_frame(name='signal').reset_index()
    plot_raw_emg.loc[:, 'time_sec'] = plot_raw_emg['time_usec'] * 1e-6
    downsampled_mask = plot_emg['time_usec'].isin(plot_emg['time_usec'].unique()[::emg_downsample])

    if x_axis_name in ['freq', 'freq_late']:
        elec_subset = ['-[26]+[27]', '-(26,)+(27,)']  ## ['-(26,)+(27,)']  #  plot_emg['elecConfig_str'].unique().tolist()   #  ['-(18,)+(26,)', '-(26,)+(27,)', '-(18,)+(27,)']
        label_subset = ['RMH']   ## plot_emg['label'].unique().tolist()  # ['RTA', 'RMG', 'RSOL']
    if x_axis_name == 'amp':
        elec_subset = plot_emg['elecConfig_str'].unique().tolist() ## ['-[26]+[27]', '-(26,)+(27,)']  # # plot_emg['elecConfig_str'].unique().tolist()
        label_subset = ['RMH']   #   plot_emg['label'].unique().tolist()
    ...
    plot_raw_emg = plot_raw_emg.loc[plot_mask, :].copy()
    ...
    relplot_estimator = np.mean
    relplot_units = None  # 'timestamp_usec'
            ...
            for hue_name, hue_group in row_group.groupby(hue_var):
                ...
                if relplot_estimator is None:
                    for sub_name, sub_group in hue_group.groupby(['block', 'timestamp_usec']):
                        ...
                        plot_raw_emg.loc[sub_group.index, 'signal'] += total_vert_offset
                        ...
                        plot_raw_emg.loc[sub_group.index, 'time_sec'] += total_horz_offset
                    ...
                    # print(f'\tsub_group.shape = {sub_group.shape}')
                    # print(f'\n{sub_group}')
                    plot_emg.loc[hue_group.index, 'signal'] += total_vert_offset
                    plot_raw_emg.loc[hue_group.index, 'signal'] += total_vert_offset
                    hline_df.loc[pd_idx[row_name, hue_name, :, :], :] += total_vert_offset
                    plot_emg.loc[hue_group.index, 'time_sec'] += total_horz_offset
                    plot_raw_emg.loc[hue_group.index, 'time_sec'] += total_horz_offset
                    total_vert_offset += vert_sub_offset
                    total_horz_offset += horz_sub_offset
                    # total_vert_offset += vert_offset
                    # total_horz_offset += horz_offset

    dummy_color = 'b' # emg_hue_map['RMH']
    dummy_palette = {freq_cutoff: dummy_color}
    print('Saving stim-triggered plots')
    g = sns.relplot(
        data=plot_raw_emg,
        col='label', row='elecConfig_str',
        x='time_sec', y='signal',
        hue=hue_var, kind='line',
        units=relplot_units,
        estimator=relplot_estimator,
        errorbar=None,
        height=1.1, aspect=1.8, facet_kws=dict(sharey=False), palette=dummy_palette,
    ...
    for ax_name, this_ax in g.axes_dict.items():
        row_name, col_name = ax_name
        selected_df = plot_emg.loc[(plot_emg[g._col_var] == col_name) & (plot_emg[g._row_var] == row_name), :]
        sns.lineplot(
            selected_df, x='time_sec', y='signal',
            hue=hue_var, palette=dummy_palette,
            units=relplot_units,
            estimator=relplot_estimator,
            errorbar=None,
            legend=False, ax=this_ax,
            linewidth=1., alpha=0.5,
        )
        this_ax.set_xlim((left_sweep * 1e-6, right_sweep * 1e-6))
        y_min, y_max = this_ax.get_ylim()
        if offset_the_traces:
            for row_idx, row in hline_df.iterrows():
                if row_idx[0] == ax_name[0]:
                    this_ax.axhline(y=row['signal'], alpha=0.1, color='k', zorder=0.1)
        # this_ax.axvspan(0, 100e-3, alpha=0.25, color='g', zorder=0.05)
        # this_ax.axvspan(100e-3, 300e-3, alpha=0.25, color='m', zorder=0.05)
        average_df = selected_df.groupby('time_sec').mean()['signal']
        zone_0 = (average_df.index > 0) & (average_df.index < 100e-3)
        this_ax.fill_between(
            average_df.index, average_df, y2=0, where=zone_0, alpha=0.1, color='g', zorder=0.05)
        this_ax.text(100e-3, y_max, 'Amplitude\nbased\nrecruitment', color='g', va='bottom', ha='right')
        zone_1 = (average_df.index > 100e-3) & (average_df.index < 300e-3)
        this_ax.fill_between(
            average_df.index, average_df, y2=0, where=zone_1, alpha=0.2, color='m', zorder=0.05)
        this_ax.text(300e-3, y_max, 'Frequency\nbased\nrecruitment', color='m', va='bottom', ha='right')
        for milestone in [0, 100e-3, 300e-3]:
            this_ax.axvline(x=milestone, alpha=0.2, color='k', zorder=0.1, ls='--')
        this_ax.set_yticklabels([])
        if not has_legend:
            # dummy_legend_handle = mpl.patches.Rectangle(
            #     (0, 0), 1, 1, fill=False, edgecolor='none', visible=False)
            custom_legend_lines = [
                mpl.lines.Line2D([0], [0], color=dummy_color, lw=1, alpha=0.5),
                mpl.lines.Line2D([0], [0], color=dummy_color, lw=.5),
            ]
            this_ax.legend(
                custom_legend_lines, ['Rectified', 'Raw'], loc='center left',
                bbox_to_anchor=(1.025, .5), borderaxespad=0.)
            has_legend = True
